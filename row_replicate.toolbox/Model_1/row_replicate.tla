...
\*     end while;
  ...
  SafeModeReplicate:
    while log_index <= Len(change_log) do
      with change = change_log[log_index] do
        if change.type = "insert" then
          downstream := Replace(downstream, change.new)
        elsif change.type = "delete" then
          downstream := DeleteByPK(downstream, change.old)
        else
          downstream := Replace(DeleteByPK(downstream, change.old), change.new)
        end if;
      end with;
...
\* BEGIN TRANSLATION (chksum(pcal) = "ae60ddda" /\ chksum(tla) = "af29247c")
                     ...
                     /\ IF log_index <= Len(change_log)
                           THEN /\ LET change == change_log[log_index] IN
                                     IF change.type = "insert"
                                        THEN /\ downstream' = Replace(downstream, change.new)
                                        ELSE /\ IF change.type = "delete"
                                                   THEN /\ downstream' = DeleteByPK(downstream, change.old)
                                                   ELSE /\ downstream' = Replace(DeleteByPK(downstream, change.old), change.new)
